...
- 🎯 **全新引擎：ComputeSharp (DirectX 12) 真实 GPU 计算**
- ✅ 自动检测 GPU 并执行着色器压力测试
- 📊 显示 GPU 型号、运算次数、FLOPs 吞吐量
- 🔄 支持 DX12 / DX11 / CPU 三级回退机制
- ⚡ 显卡驱动正常即可工作，无需额外安装

### v1.2.0 (2026-04-27)

- 🎯 **真正调用 GPU 计算！** 使用 OpenCL 执行真实 GPU 矩阵乘法
- ✅ 自动检测 GPU 设备和 OpenCL 支持
- 📊 实时显示 GPU 运算次数和浮点运算量
- 🔄 无 OpenCL 时自动回退到 CPU 模拟模式
- 🐛 修复内存测试超时后不停止的问题
- ⚡ 优化内存测试轮次显示
